  ...
  <div class="content">
    ...
    <md-card>
      <md-card-header data-background-color="green">
        <h4 class="title">Edit New Invoice</h4>
      </md-card-header>
      <md-card-content>
        <div>
          </md-button>
          ...
          <div class="md-layout" v-for="(row, index) in rows" :key="index">
             ...
             <div class="md-layout-item md-small-size-100 md-size-10">
               <md-field>
                 <label>Bonus</label>
                 <md-input v-model="row.bonus" type="number" readonly></md-input>
               </md-field>
             </div>
             <div class="md-layout-item md-small-size-100 md-size-10">
               <md-field>
          ...
          <md-button @click="active = true" class="md-success">Update</md-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rows: [{'product':null,'qty':null,'bonus':null,'total':null,'discount':null,'disctotal':null,'unit_price':0}],
    ...
    mounted: function() {
      ...
      axios.get('invoice/'+this.id+'/edit')
      .then(d => {this.rows = d.data.sales;
        console.log(d.data.invoice.customer);
        this.customer = d.data.invoice.customer;
        this.saleman = d.data.invoice.saleman;
        this.orderbooker = d.data.invoice.orderbooker;
        this.invoicedate = d.data.invoice.invoicedate;
        })
    ...
    methods: {
      ...
      update () {
        axios.post('/invoice',{'id':this.id,'_method':'PUT','productrows':this.rows,'customer':this.customer,'orderbooker':this.orderbooker,'saleman':this.saleman,'invoicedate':this.invoicedate})
